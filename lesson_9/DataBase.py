...
class DataBase:
    query = {
        'create_company' : text('insert inro company (name, description) values (:name, :description)'),
...
#Получаю ID последней созданной компании из БД
...
#Получаю список сотрудников последней созданной компании из БД
...
#Получаю ID нового сотрудника в БД
def get_employer_id(self, company_id: int):
    try:
        with self.bd.connect() as connection:
            result = connection.execute(self.query['maxID_SELECT'], parameters = dict(id=company_id)).fetchall()[0][0]
            return result
    except Exception as _ex:
        print("[INFO] Error - can't work with SQL", _ex)
    finally:
...
#Изменяю информацию о сотруднике в БД
